...
public class DTStreamingHistogram<E extends AbstractPointable> implements IDTHistogram<E> {
    ...
    private double pointableToQuantile(E item) throws HyracksDataException {
        switch (type) {
            case SHORT:
                return SampleUtils.shortMappingToQuantile(item);
            case INT:
                return SampleUtils.integerMappingToQuantile(item);
            case LONG:
                return SampleUtils.longMappingToQuantile(item);
            case FLOAT:
                return SampleUtils.floatMappingToQuantile(item);
            case DOUBLE:
                return SampleUtils.doubleMappingToQuantile(item);
            case UTF8:
                return SampleUtils.ansiMappingToQuantile(item, 0, DEFAULT_ANSI_SAMPLE_LENGTH);
            default:
    ...
    private E quantileToPointable(double d) throws HyracksDataException {
        switch (type) {
            case SHORT:
                return (E) SampleUtils.quantileRevertToShort(d);
            case INT:
                return (E) SampleUtils.quantileRevertToInteger(d);
            case LONG:
                return (E) SampleUtils.quantileRevertToLong(d);
            case FLOAT:
                return (E) SampleUtils.quantileRevertToFloat(d);
            case DOUBLE:
                return (E) SampleUtils.quantileRevertToDouble(d);
            case UTF8:
                return (E) SampleUtils.quantileRevertToAnsi(d, DEFAULT_ANSI_SAMPLE_LENGTH);
            default:
